...
This function needs to look at a grid of numbers and calculate the standard deviation for each column.
...
Go down the column to add up all the numbers and calculate the average (mean).
Go down the same column again to see how far each number is from that average, square those differences, and add them up.
Finally Divide that total by the number of rows (minus 1) and use std::sqrt() to get the final answer.
